...
function SelectUser() {
    ...
    let onSelectUser = (user) => dispatch(setUser(user));
    let { users, isLoaded, userPageCount, userPage } = useSelector(({ users }) => users);
    ...
    return (
        <section className="selectUser">
            ...
            <div className="selectUser__wrapper">
                {isLoaded ?
                    users.map((el) => !el.selected && < SelectUserItem key={el.id} user={el} onSelectUser={onSelectUser} />)
